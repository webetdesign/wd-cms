...
namespace WebEtDesign\CmsBundle\Security;
...
class EditableRolesBuilder
{
    ...
    {
        $cmsPageRoles = !empty($this->cmsConfig['security']['page']['roles']) ? $this->cmsConfig['security']['page']['roles'] : null;
        ...
        foreach ($this->rolesHierarchy as $name => $rolesHierarchy) {
            if ($cmsPageRoles === null || $cmsPageRoles !== null && in_array($name, $cmsPageRoles)) {
                if ($this->authorizationChecker->isGranted($name) || $isMaster) {
                    $roles[$name] = $this->translateRole($name, $domain);
                    if ($expanded) {
                        $result       = array_map([$this, 'translateRole'], $rolesHierarchy,
